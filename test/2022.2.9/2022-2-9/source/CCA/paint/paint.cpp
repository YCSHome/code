...
int main () {
    ...
    int num = 0;
    rep(i, 1, n) rep(j, 1, m)
        if (!tag[col[i][j]]) tag[col[i][j]] = true, num++;
    ...
    rep(i, 1, n) rep(j, 1, m) {
        if (a[col[i][j]] == -1) a[col[i][j]] = i;
        else a[col[i][j]] = min(a[col[i][j]], i);
        if (c[col[i][j]] == -1) c[col[i][j]] = i;
        else c[col[i][j]] = max(c[col[i][j]], i);
        if (b[col[i][j]] == -1) b[col[i][j]] = j;
        else b[col[i][j]] = min(b[col[i][j]], j);
        if (d[col[i][j]] == -1) d[col[i][j]] = j;
        else d[col[i][j]] = max(d[col[i][j]], j);
    }
...
（事实上还可以随机化，可以做到 O[T(nm + k)]，T 为随机次数，毛估估一下大概 3 - 4 ?）
---------------------------------------------------------------------*/
    ...
    rep(t, 0, 17) {
        ...
        rep(p, 0, 1) rep(i, 1, n) rep(j, 1, m)
            sum[p][i][j] += sum[p][i - 1][j] + sum[p][i][j - 1] - sum[p][i - 1][j - 1];
        rep(i, 1, n) rep(j, 1, m)
            if (sum[(col[i][j] >> t) & 1][i][j] > 0) tag[col[i][j]] = true;
    }
